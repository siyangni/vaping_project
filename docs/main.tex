...
\begin{abstract}
\textbf{Objectives:} Develop and validate a methodological framework integrating machine learning (ML) with generalized linear regression to identify complex predictors of adolescent nicotine vaping. Traditional regression struggles with high-dimensional interactions; single ML models risk algorithm-specific artifacts; pure ML lacks interpretable inference for policy. We demonstrate a three-stage solution: diverse ML expert system for robust feature discovery, cross-model consensus for selection, and nested regression for interpretable inference.

\textbf{Methods:} Using Monitoring the Future data (2017-2023, $N=72,712$ 12th-graders, 123 features), we trained six algorithms (Lasso, Random Forest, Gradient Boosting, Histogram-based GB, XGBoost, CatBoost) to predict nicotine vaping. SHAP values identified feature importance consensus across models. Features were ranked by cross-model agreement and incrementally tested via six-step nested weighted logistic regression with multiple imputation and survey design adjustments.

\textbf{Results:} Tree-based models achieved ROC AUC greater than 0.90 (range: 0.909-0.916), substantially outperforming Lasso (0.743). Cross-model consensus identified survey wave as dominant predictor with dramatic 2020-2021 threshold effect: vaping probability doubled post-COVID-19 onset (OR=1.64 per year, 95 percent CI [1.61, 1.67], $p<0.001$). Contrary to polysubstance theory, marijuana (OR=0.90, $p<0.001$) and alcohol (OR=0.94, $p<0.001$) use negatively predicted vaping, suggesting substance substitution rather than complementarity. All consensus features achieved statistical significance in final regression ($p<0.05$), with optimal consensus at features important to at least 50 percent of models. Temporal validation (train 2017-2021, test 2022-2023) showed 3-4 percent AUC degradation, confirming robust generalization.

\textbf{Conclusions:} This integrated framework successfully identified: (1) pandemic-era structural break in adolescent substance use, (2) counterintuitive protective effects of other substance use contradicting gateway theories, and (3) complex interactions impossible to specify a priori. The multi-model ensemble provided more robust feature selection than theory-driven or single-algorithm strategies. Findings demonstrate value of abductive analytical paradigm combining ML discovery with traditional inference, advancing quantitative criminology's capacity to handle complex, high-dimensional data while maintaining interpretability for policy communication.
\end{abstract}
...
\subsubsection{Incorporating Discovered Interactions into Regression}

When SHAP analysis reveals consistent interactions across models, these should be explicitly tested in the regression stage. For interactions appearing in top-10 SHAP interaction rankings across at least four of six models, we specify:

\begin{equation}
\text{logit}(P(Y=1)) = \beta_0 + \boldsymbol{\beta}^\top\mathbf{X} + \sum_{(j,k) \in \mathcal{I}} \gamma_{jk} X_j X_k
\end{equation}

where $\mathcal{I}$ is the set of consensus interactions. For our vaping application, this yields:

\begin{equation}
\text{logit}(P(\text{Vaping}=1)) = \beta_0 + \beta_1 \text{Wave} + \beta_2 \text{MJ} + \beta_3 \text{Alc} + \gamma_1 (\text{Wave} \times \text{MJ}) + \gamma_2 (\text{Wave} \times \text{Alc}) + \ldots
\end{equation}

We fit three regression model variants: Model A (main effects only, original nested models 1-6), Model B (plus two-way interactions identified via SHAP consensus), and Model C (plus threshold effects incorporating period indicator $I(\text{Wave} > 2020)$ and interactions). Likelihood ratio tests quantify whether interactions significantly improve model fit, validating the ML discovery process.
...
\paragraph{Formal Statistical Test for Structural Break}

We formally tested the 2020-2021 threshold using a Chow test for structural change. The test compares three models: (1) pooled model assuming no break, (2) separate pre-2021 model, and (3) separate post-2020 model. The test statistic is:

\begin{equation}
\text{LR} = -2(\log L_{pooled} - (\log L_{pre} + \log L_{post}))
\end{equation}

Results: $\text{LR}(7) = 1,847.32$, $p < 0.001$, strongly rejecting the null of no structural break. The post-2020 period shows a fundamentally different data-generating process. Interrupted time series analysis confirmed a significant level shift (intercept change) and slope change at 2021, with the marijuana protective effect changing from OR=0.85 (pre-2021) to OR=1.05 (post-2020), supporting our interpretation that pandemic disrupted established substance use patterns.
...
Our framework identifies associations, not causal effects. Three specific threats to causal interpretation merit discussion:

\textbf{Limitation 1: Reverse causality in polysubstance relationships.} The negative marijuana-vaping association could reflect that: (a) vaping reduces marijuana use (substitution), (b) marijuana use reduces vaping uptake, or (c) both are caused by unobserved third variables (peer group selection). Cross-sectional data cannot adjudicate between these mechanisms.

\textit{Concrete remedy:} MTF includes longitudinal panels. Future research should employ fixed-effects models:
\begin{equation}
Y_{it} = \alpha_i + \beta X_{it} + \gamma_t + \epsilon_{it}
\end{equation}
where $\alpha_i$ controls time-invariant confounders. This would test whether within-person increases in marijuana use predict subsequent decreases in vaping, supporting substitution.

\textbf{Limitation 2: Omitted variable bias in COVID-19 effect.} The 2020-2021 break coincides with pandemic onset, but numerous confounded changes occurred simultaneously: school closures, economic disruption, social isolation, changes in enforcement. Our data cannot isolate which mechanism(s) drove the vaping surge.

\textit{Concrete remedy:} Exploit state-level variation in pandemic policies. States varied substantially in school closure duration (0-52 weeks), unemployment levels, and retail enforcement. A difference-in-differences design leveraging this variation could identify causal mechanisms:
\begin{equation}
Y_{ist} = \beta_1 Post_{st} + \beta_2 (Post_{st} \times SchoolClosure_{s}) + \alpha_i + \delta_s + \gamma_t + \epsilon_{ist}
\end{equation}

\textbf{Limitation 3: Selection into the sample.} MTF excludes school dropouts, absentees on survey day, and homeschooled youth---populations with elevated substance use risk. This creates non-random sample selection potentially biasing all estimates downward.

\textit{Concrete remedy:} Heckman selection models or inverse probability weighting using demographic predictors of school enrollment. Administrative data linkages (juvenile justice records, emergency department visits) could provide substance use estimates for out-of-school youth for sensitivity analysis.

Recent developments in causal machine learning (e.g., double/debiased machine learning, causal forests) could be integrated into this framework's first stage for more rigorous causal inference.
